...
**دستیار هوشمند مدیریت مالی برای کسب‌وکارهای کوچک و متوسط ایرانی**

خیلی از صاحبان کسب‌وکار وقت و تخصص یک حسابدار حرفه‌ای را ندارند، اما باید همیشه بدانند «الان چقدر پول داریم؟» و «تا کِی دوام می‌آوریم؟». **پیش‌بین** دقیقاً همین کار را می‌کند: حساب‌ها، تراکنش‌ها، چک‌ها و بودجه‌ها را در یک‌جا جمع می‌کند و با یک نگاه ساده نشان می‌دهد وضعیت نقدینگی کسب‌وکار در ۳۰ روز آینده چطور خواهد بود.

پروژه از دو بخش تشکیل شده: **بک‌اند** (سرور و پایگاه‌داده، با FastAPI) و **فرانت‌اند** (رابط کاربری وب، با Next.js).

## چه کارهایی می‌توان با پیش‌بین انجام داد؟

- 🔐 ورود امن به حساب کاربری، همراه با **تأیید دومرحله‌ای** برای امنیت بیشتر
- 🏢 مدیریت چند کسب‌وکار یا شعبه با یک حساب کاربری
- 💳 ثبت و مدیریت حساب‌های بانکی (جاری، پس‌انداز، کارت اعتباری و ...)
- 🧾 ثبت و دسته‌بندی تراکنش‌های درآمد و هزینه
- 📊 تعیین بودجه برای هر دسته‌بندی و پیگیری میزان خرج نسبت به آن
- 🎯 تعریف اهداف مالی (مثلاً پس‌انداز) و دیدن پیشرفت آن‌ها
- 📩 خواندن خودکار پیامک‌های بانکی و تبدیل آن‌ها به تراکنش
- 🔁 مدیریت تراکنش‌های تکرارشونده مثل اجاره یا حقوق ماهانه
- 🧿 ثبت و پیگیری چک‌های صادرشده و دریافتی
- 🔗 تطبیق خودکار صورت‌حساب بانکی با تراکنش‌های ثبت‌شده
- 💰 پرداخت آنلاین امن با درگاه زرین‌پال
- 🔔 هشدارهای مالی و امکان پشتیبان‌گیری از اطلاعات
- 📈 داشبورد ساده با خلاصه وضعیت مالی، گزارش‌ها و پیش‌بینی هزینه‌ها
...
فهرست کامل و دقیق همه مسیرهای API در **[docs/API_SPEC.md](docs/API_SPEC.md)** موجود است. چند نمونه از مهم‌ترین‌ها:
...
- `GET/POST /api/v1/checks` — ثبت/مشاهده چک (صادره/دریافتی)، `GET /api/v1/checks/cash-flow-forecast` برای چک‌های در جریان تا N روز آینده
- `POST /api/v1/reconciliation/bank-statement` — آشتی‌سازی خودکار صورت‌حساب بانکی (CSV) با تراکنش‌های ثبت‌شده و گزارش ردیف‌های تطبیق‌نیافته
